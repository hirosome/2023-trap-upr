...
## Step1
# Input filenames
ribo_file = "input_txt/P-TRAP_DMSOvsTPG_All.txt"
rna_file = "input_txt/Total_DMSOvsTPG_All.txt"
data_file = "input_txt/sampleInfo.txt"
batch = 1
...
sizeFactors(ddsMat)
sizeFactors = sizeFactors(ddsMat)

#################################################################################################################
# T>CリードでdeltaTE解析を行う場合はこの範囲を実行
# Input filenames
ribo_file = "input_txt/P-TRAP_DMSOvsTPG_TC.txt"
rna_file = "input_txt/Total_DMSOvsTPG_TC.txt"
...
ddsMat$SeqType = relevel(ddsMat$SeqType,"RNA")
ddsMat = estimateSizeFactors(ddsMat)
sizeFactors(ddsMat) = sizeFactors # T>Cリードではなく全リードで求めたsizeFactorsを使用
ddsMat = estimateDispersions(ddsMat)
ddsMat = nbinomWaldTest(ddsMat)
resultsNames(ddsMat)
#################################################################################################################
...
# setwd("Results")
system("mkdir Results/fold_changes")
system("mkdir Results/gene_lists")
...
write.table(rownames(res)[which(res$padj < 0.05)],"Results/gene_lists/DTEGs.txt",quote=F,sep="\t",col.names = F,row.names = F)
write.table(res,"Results/fold_changes/deltaTE.txt",quote=F,sep="\t",col.names = T,row.names = T)

pdf("Results/Result_figures.pdf",useDingbats = F)
...
# size factorを求める
ddsMat_ribo = estimateSizeFactors(ddsMat_ribo)
sizeFactors(ddsMat_ribo)
sizeFactors = sizeFactors(ddsMat_ribo)
#################################################################################################################
...
ddsMat_ribo = estimateSizeFactors(ddsMat_ribo)
sizeFactors(ddsMat_ribo) = sizeFactors # T>Cリードではなく全リードで求めたsizeFactorsを使用
ddsMat_ribo = estimateDispersions(ddsMat_ribo)
ddsMat_ribo = nbinomWaldTest(ddsMat_ribo)
...
write.table(res_ribo,"Results/fold_changes/deltaRibo.txt",quote=F,sep="\t",col.names = T,row.names = T)
...
# size factorを求める
ddsMat_rna = estimateSizeFactors(ddsMat_rna)
sizeFactors(ddsMat_rna)
sizeFactors = sizeFactors(ddsMat_rna)
...
ddsMat_rna = estimateSizeFactors(ddsMat_rna)
sizeFactors(ddsMat_rna) = sizeFactors # T>Cリードではなく全リードで求めたsizeFactorsを使用
ddsMat_rna = estimateDispersions(ddsMat_rna)
ddsMat_rna = nbinomWaldTest(ddsMat_rna)

res_rna <- results(ddsMat_rna, contrast=c("Condition","2","1"))
res_rna <- lfcShrink(ddsMat_rna, coef=2,type="apeglm",res=res_rna)
write.table(res_rna,"Results/fold_changes/deltaRNA.txt",quote=F,sep="\t",col.names = T,row.names = T)
write.table(rownames(res_rna)[which(res_rna$padj < 0.05)],"Results/gene_lists/DTG.txt",quote=F,sep="\t",col.names = F,row.names = F)



## Classes of genes
forwarded = rownames(res)[which(res$padj > 0.05 & res_ribo$padj < 0.05 & res_rna$padj < 0.05)]
write.table(forwarded,"Results/gene_lists/forwarded.txt",quote=F,sep="\t",col.names = F,row.names = F)
exclusive = rownames(res)[which(res$padj < 0.05 & res_ribo$padj < 0.05 & res_rna$padj > 0.05)]
write.table(exclusive,"Results/gene_lists/exclusive.txt",quote=F,sep="\t",col.names = F,row.names = F)
...
write.table(intensified,"Results/gene_lists/intensified.txt",quote=F,sep="\t",col.names = F,row.names = F)
...
write.table(buffered,"Results/gene_lists/buffered.txt",quote=F,sep="\t",col.names = F,row.names = F)
